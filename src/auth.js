...
const AUTH_BASE_DIR = (
   process.env.CONDON_AUTH_BASE_DIR?
   i_path.resolve(process.env.CONDON_AUTH_BASE_DIR):null
);
const AUTH_SESSION_TIMEOUT = parseInt(
   process.env.CONDON_ATUH_TIMEOUT || `${24 * 3600}`
) * 1000 || Infinity;
if (!AUTH_BASE_DIR) {
   console.warn('[!] CONDON_AUTH_BASE_DIR is empty: authentication module not work');
}
