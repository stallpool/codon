...
function set_cookie(key, value) {
   ...
   document.cookie = key + '=' + escape(value) + ';path=/;domain=' + window.codonenv.hostname;
}
...
function update_uuid(jsonData) {
   ...
   if (obj._sessionId) {
      set_cookie('codon_uuid', obj._sessionId);
      delete obj._sessionId;
   }
   return obj;
}
...
function detect_uuid_change(data) {
   ...
   var obj = JSON.parse(data);
      if (obj.sessionId) set_cookie('codon_uuid', obj.sessionId);
   } catch (err) {}
}
...
function get_auth_header() {
   ...
   return { 'auth-token': (
      cookie.codon_user + '/' + cookie.codon_uuid
   )}
}
